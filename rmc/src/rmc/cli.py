...
from .exporters.tldraw import tree_to_tldraw, tree_to_tldraw_raw
import logging
...
def cli(verbose, from_, to, output, input):
    ...
    `tldraw`, `tldraw-raw`, `inkml`, `blocks`, `blocks-data`.
    ...
    `tldraw` exports to TLDRaw JSON format for tldraw applications.
    `tldraw-raw` exports raw stroke data with minimal processing.
    `inkml` exports to InkML format for OneNote compatibility.
...
def guess_format(p: Path):
    ...
    if p.suffix == ".svg":
        ...
        # Check if filename suggests tldraw format
        if "tldraw" in p.name.lower():
            return "tldraw"
        else:
            return "blocks"
    else:
...
def convert_rm(filename: Path, to, fout: typing.IO):
    with open(filename, "rb") as f:
        if to == "blocks":
            ...
            tree = read_tree(f)
            tree_to_tldraw(tree, fout)
        elif to == "tldraw-raw":
            tree = read_tree(f)
            tree_to_tldraw_raw(tree, fout)
        else:
